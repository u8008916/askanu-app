...
## 8. Final sync check against the merged RAG contract (2026-09-17)

Before Day 13, checked PR #34 directly against `askanu-rag` `main` at `ccd93b3` ("feat: align Accommodation and Support frozen contracts", PR #29, merged 2026-09-17), the actual merged contract behind Qasim's frozen-identity direction — not just the chat summary of it. Read `src/askanu_rag/models/records.py`'s `CommonRecord`/`AccommodationRecord`/`SupportRecord` validators directly.

**Result: exact match, no further App change needed.** `source_id` literals (`accommodation_anu_study`, `support_anusa_student_assistance`), `record_id` format (`{domain}:{entity_type}:{entity_id}` → `accommodation:residence:<slug>` / `support:support_service:<slug>`) and `domain` literals (`accommodation`, `support`) in this repo's fixtures (§1, this pass's prior commit) match the frozen `CommonRecord` validators byte for byte. RAG's own canonical evidence-record URLs use the same two approved hosts (`study.anu.edu.au/accommodation/our-residences/<slug>`, `anusa.com.au/student-assistance/<slug>/`) already used for this repo's official-navigation resource links (§2) — no host-list change needed either. RAG's newly-documented structured room/contact/topic/referral shapes and source-backed-only vacancy handling are internal retrieval structures folded into the synthesised `answer` text; the `/api/v1/ask` envelope and `sources[]` shape the App renders are unchanged, and RAG's own `docs/API_CONTRACT.md` still shows `items: []` for `/ask`. Recorded durably in this repo's `docs/DECISION_LOG.md` (2026-09-17 entry).

**Fail-honestly behaviour:** RAG's own Day 12 handoff (`askanu-rag docs/V6_DAY12_ACCOMMODATION_SUPPORT_HANDOFF.md`) documents "high-stakes fail-closed answers for vacancy, diagnosis, emergency coverage, response time, 24/7 and professional availability" and "vacancy is source-backed only; null remains unknown." The App side of this was already verified in §5: `insufficient_evidence`/`error` render the existing controlled `StatusNotice` with zero invented sources or content. **Caveat:** this environment has no live connection to Cloud SQL/the deployed RAG service, so end-to-end verification here is against contract-shaped mocks only (same method as the rest of this evidence file), not a live RAG call — label any claim of Accommodation/Support "working" accordingly until an integrated environment is available.

**Known cross-repo gap, not App's to fix:** RAG's handoff doc flags that the scraper's Support Referral validator still accepts internal ANUSA/credential-bearing URLs, while the frozen contract requires external, credential-free Referral destinations. This blocks nothing on the App side (the App never validates or constructs Referral URLs — it only renders what `sources[]`/`answer` sends) but is worth Qasim/Will's attention before Support goes live with real data.

**Re-verified with this SHA:** `server npm test` 46/46; `frontend npm run test` 243/243 (18 files); `frontend npm run build` clean, `dist/assets/index-*.js` 274.06 kB / gzip 87.15 kB, 0 leaked fixture strings.
